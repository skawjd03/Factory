var CropImage = (function() {
	...
	function demoUpload() {
		...
		$('#uploadCandy').on('click', async function (ev) {
			var resp = await 	$uploadCrop.croppie('result', {
				type: 'canvas',
				size: 'viewport'
			});
				$.ajax({
					...
					success: function(result) {
						$('#miniImg').append(`<img src='${resp}' width='100%; height='100%'; style='display:inline-block;'>`);
						$('.uploadTo').on('click',function(e){
							$.ajax({
								type:'post',
								url:'/post',
								data:{'hashtags':$('#hashtags').val(),'img':result},
								success: function(rs){
									popupClose();
									location.href='/';
								}
